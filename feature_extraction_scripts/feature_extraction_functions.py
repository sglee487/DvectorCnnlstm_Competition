...
def get_cqt(y, sr, window_size=None, window_shift=None):
    if window_size is None:
        n_fft = int(0.025*sr)
    else:
        n_fft = int(window_size*0.001*sr)
    ...
    # C = librosa.cqt(y,sr,fmin=fmin,n_bins=72,hop_length=hop_length)
    C = librosa.cqt(y,sr=sr,fmin=fmin,n_bins=72,hop_length=hop_length)

    cqt = librosa.amplitude_to_db(np.abs(C))
    cqt = np.transpose(cqt)
    #cqt = np.transpose(cqt)
    cqt -= (np.mean(cqt, axis=0) + 1e-8)
