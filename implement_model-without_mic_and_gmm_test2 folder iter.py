...
def main(project_head_folder, model_name):
    ...
    models_gmm = [cPickle.load(open(fname, 'rb'), encoding="utf-16") for fname in gmm_files]
    ...
    for speech_filename in os.listdir("recording_mic"):
        ...
        path = str(speech_filename).strip()
        print("Testing Audio : ", path)
        # sr,audio = read(source + path) # from scipy.io.wavfile import read. return sample rate, data numpy array
        # sr, audio = read(path)  # from scipy.io.wavfile import read. return sample rate, data numpy array
        y, sr = librosa.load(speech_filename, sr=16000)
        print("sound dB : " + str(np.mean(np.sqrt(np.square(y)))))
        if (np.mean(np.sqrt(np.square(y)))) < 0.001:
            print("Too LOW dB.")
            print("GMM check end with FILTER")
            continue
        # vector = extract_features(y,sr)
        vector = get_mfcc(y, sr)
        # vector = get_stft(y, sr)

        log_likelihood = np.zeros(len(models_gmm))

        for i in range(len(models_gmm)):
            gmm = models_gmm[i]  # checking with each model one by one
            scores = np.array(gmm.score(vector))
            log_likelihood[i] = scores.sum()

        print(log_likelihood)
        print(log_likelihood.std())
        ...
        # criteria_score = -24.8  # when use extract_features(y,sr)
        criteria_score = -35000  # when use get_mfcc(y, sr)

        print(max(log_likelihood))
        if max(log_likelihood) < criteria_score:
            print("This person is not permitted maxscore.")
            print("GMM check end with FILTER")
        ...
        # criteria_std = 3.3  # when use extract_features(y,sr)
        criteria_std = 7500  # when use get_mfcc(y, sr)

        if log_likelihood.std() < criteria_std:
            print("This person is not permitted std.")
            print("GMM check end with FILTER")
            if is_he_on_list:
                identification_try_fail += 1
            else:
